...
export default function Videos() {
  return (
    <section
      id="videos"
      className="py-16 px-3 text-[#f0ece2] poj-blur-bg poj-blur-bg-4 section-boundary"
    >
      <div className="mx-auto max-w-screen-xl space-y-8">
        <div className="max-w-3xl space-y-3">
          <p className="text-sm uppercase tracking-[0.3em] text-[#d9d4ca]">
            Live Performances
          </p>
          <h2 className="text-4xl sm:text-5xl font-semibold">Watch P-O-J Legacy in action</h2>
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 snap-x snap-mandatory lg:gap-6 lg:pb-0 lg:flex-nowrap lg:overflow-x-auto lg:overflow-y-hidden">
          {videoEmbeds.map((src, index) => (
            <div
              key={src}
              className="min-w-[85%] snap-center overflow-hidden rounded-3xl border border-white/15 bg-white/5 shadow-2xl sm:min-w-[48%] lg:min-w-[42%] xl:min-w-[32%]"
            >
